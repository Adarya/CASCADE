            ...
            try:
                ...
                if self._gate_evaluator is not None and layer_result.succeeded:
                    ...
                    if not gate_result.passed:
                        logger.warning(
                            f"Layer {name} gate FAILED: {gate_result.error_message}"
                        )
                        layer_result.warnings.append(
                            f"Gate FAILED: {gate_result.error_message}"
                        )

                        # Attempt remediation if strategy exists
                            ...
                            for attempt in range(self._max_retries):
                                ...
                                try:
                                    ...
                                    if gate_result_2.passed:
                                        ...
                                        logger.info(
                                            f"Remediation succeeded for layer {name}"
                                        )
                                        break
                                    else:
                                        logger.info(
                                    ...
                                    logger.error(
                                        f"Remediation failed for {name}: {rem_e}"
                                    )
                                    break
